...
class RC{
  ...
  morphRows(rows,newSym,dur,staggerMs,done){
    ...
    const frame=now=>{
      const elapsed=window._skipSpin ? (now-t0)*SKIP_TIME_MULT : (now-t0);
  ...
  triggerStop(finalSyms,tensionMode,done){
    ...
    const frame=now=>{
      const elapsed=window._skipSpin ? (now-t0)*SKIP_TIME_MULT : (now-t0);
      const t=Math.min(elapsed/decelDur,1);
...
function playExpandMorph(reelIdxs,fsym,allDone){
  ...
  function startReel(i){
    ...
    if(i<reelIdxs.length-1){
      setTimeout(()=>startReel(i+1), window._skipSpin?Math.max(20,Math.round((perReelTime+REEL_GAP)/SKIP_TIME_MULT)):(perReelTime+REEL_GAP));
    }else{
      ...
      setTimeout(()=>{
        ...
        setTimeout(()=>{
          ...
          _finish();
        },window._skipSpin?Math.max(20,Math.round(500/SKIP_TIME_MULT)):500);
      },window._skipSpin?Math.max(20,Math.round((perReelTime+60)/SKIP_TIME_MULT)):(perReelTime+60));
    }
  }
  startReel(0);
}
